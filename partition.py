...
if sys.version_info >= (3,0):
    ...
    def logger(output):
        def log_print(*args, **kwargs):
            nprint(*args, **kwargs)
            return nprint(file=open(output, 'a'), flush=True, *args, **kwargs)
        return log_print
    ...
    def logger(output):
        def log_print(*args, **kwargs):
            ...
            return __builtin__.print(file=open(output, 'a'), *args, **kwargs)
        return log_print
...
class MBRTable:
    ...
    def pprint(self):
        if self.info:
            print('MBR Header')
            print('-'*88)
            print('Файл дампа      | N разд. | ФС / потенциальная ФС |  Начало  |   Размер   | Примечание |')
            print('                |         |                       |          |            | *-активный |')
            print('-'*88)
            print(' {0: <15}| Serial: 0x{1: <59}|'.format(self.filename, hex(self.info.header.drive_signature)))
            for part in self.info.partitions:
                print('-' * 88)
                print('                |{n: ^9}|{type: ^23}|{from_s: ^10}|{size_s: ^12}| {boot: <2}ID=0x{code: <4X}|'.format(n=part.index,
...
class GPTTable:
    ...
    def pprint(self):
        if self.info:
            print('GPT Header')
            print('-' * 137)
            print('Файл дампа      | N разд. |         ID разд. (GUID/UID)          |  Начало  |   Размер   |      ФС / потенциальная ФС      | Примечание |')
            print('-' * 137)
            print(' {0: <15}| GUID: 0x{1: <110}|'.format(self.filename, self.info.disk_guid))
            for part in self.info.partitions:
                print('-'*137)
                print('{empty: ^16}|{n: ^9}|{guid: ^38}|{from_s: ^10}|{size: ^12}| {type:31.31} |{flags: ^12}|'.format(empty='', n=part.index, guid=part.guid, type=part.type,
                                                                                              size=part.last_lba-part.first_lba,
                                                                                              from_s=part.first_lba, flags=part.flags))
                print('{empty: ^16}|{empty: ^9}|{uid: ^38}|{empty: ^10}|{empty: ^12}| {empty:31.31} |{empty: ^12}|'.format(empty='', uid=part.uid))
            print('-' * 137)
        else:
            print('No GPT')
        print('---')
...
class DisklabelTable:
    ...
    def read_disklabel_partitions(self, lba_size=512):
        parts = []
        fmt, DisklabelPartition = make_fmt('DisklabelPartition', DISKLABEL_PARTITION_FORMAT, extras=['index', 'type_str'])
    ...
    def pprint(self):
        if self.info:
            ...
            for part in self.info.partitions:
                print('-' * 88)
                print('                |{n: ^9}|{type: ^23}|{from_s: ^10}|{size_s: ^12}| ID=0x{code: <4X}  |'.format(n=indexer[part.index],
